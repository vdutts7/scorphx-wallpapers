...
const Home: NextPage = ({ images }: { images: ImageProps[] }) => {
  ...
  return (
    <>
      <Head>
        <title>Scorphx | Futbol Concepts </title>
        <meta
          property="og:image"
          content=""
        />
      ...
      <main className="mx-auto max-w-[1960px] p-4">
        ...
        <h1 className="mt-4 mb-2 text-3xl font-bold uppercase tracking-widest">
          PS5 Concepts
            ...
            <span className='justify-start flex flex-row '>
              <a
                className="pointer z-10 mt-6  rounded-lg border border-[1px] bg-transparent hover:bg-[#fff] px-3 py-2  md:mt-4 "
                href="/"
                rel="noreferrer"
              >
                <MoveLeft className="stroke-[#fff] hover:stroke-[#000] " size={18} />
              </a>
              {/* <a
...
export async function getStaticProps() {
  const results = await cloudinary.v2.search
    .expression(`folder:${process.env.PS5_FOLDER}/*`)
